...
// ProgressEvent reports file-level execution and the current accumulated LLM usage.
...
type Args struct {
	...
	// OnProgress receives file start/completion/failure events during a run.
	OnProgress func(ProgressEvent)

	// OnUsage receives aggregate token usage after each completed LLM call.
	OnUsage func(llmloop.UsageSnapshot)
...
func (a *Agent) Run(ctx context.Context) ([]model.LlmComment, error) {
	...
	if analysisErr := a.generateChangeAnalysis(ctx); analysisErr != nil {
		err = errors.Join(err, analysisErr)
	}
	if len(comments) > 0 {
	...
	if reconcileErr := a.reconcileHistoricalFindings(ctx); reconcileErr != nil {
		a.recordWarning("finding_reconciliation", "", reconcileErr.Error())
	}
	if ferr := a.session.Finalize(); ferr != nil {
...
// ChangeAnalysis returns the LLM-generated module, operations, and test-scope conclusion.
func (a *Agent) ChangeAnalysis() string { return a.changeAnalysis }
...
func (a *Agent) dispatchSubtasks(ctx context.Context) ([]model.LlmComment, error) {
	...
	totalSelected := countDispatchable(a.diffs)
	var completedFiles atomic.Int64
	emitProgress := func(kind, path string) {
		if a.args.OnProgress != nil {
			a.args.OnProgress(ProgressEvent{
				Kind: kind, Path: path, Completed: completedFiles.Load(), Total: totalSelected,
				InputTokens: a.TotalInputTokens(), OutputTokens: a.TotalOutputTokens(), TotalTokens: a.TotalTokensUsed(),
			})
		}
	}
	...
	for i := range toDispatch {
		...
		emitProgress("file_started", toDispatch[i].NewPath)
		go func(d model.Diff) {
			...
			defer func() {
				if r := recover(); r != nil {
					...
					emitProgress("file_failed", d.NewPath)
				}
			}()
			...
			emitProgress("file_completed", d.NewPath)
...
func (a *Agent) applyResume(diffs []model.Diff) []model.Diff {
	...
	for _, d := range diffs {
		...
		a.session.RecordReviewItemReused(effectivePath(d), d.OldPath, d.NewPath, fingerprint, sourceSessionID, item.Comments)
		a.markReused(d)
		reused++
	}
...
func (a *Agent) generateChangeAnalysis(ctx context.Context) error {
	task := a.args.Template.ChangeAnalysisTask
	if task == nil || len(task.Messages) == 0 || len(a.diffs) == 0 {
		return nil
	}
	changedFiles, diffs := a.changeAnalysisInput()
	messages := make([]llm.Message, 0, len(task.Messages))
	for _, message := range task.Messages {
		content := message.Content
		content = strings.ReplaceAll(content, "{{requirement_background}}", a.args.Background)
		content = strings.ReplaceAll(content, "{{changed_files}}", changedFiles)
		content = strings.ReplaceAll(content, "{{diffs}}", diffs)
		messages = append(messages, llm.NewTextMessage(message.Role, content))
	}
	record := a.session.GetOrCreateFileSession("__change_analysis__").AppendTaskRecord(session.ChangeAnalysisTask, messages)
	started := time.Now()
	maxTokens := a.args.Template.CompletionTokenLimit()
	if maxTokens > 3000 {
		maxTokens = 3000
	}
	resp, err := a.args.LLMClient.CompletionsWithCtx(ctx, llm.ChatRequest{Model: a.args.Model, Messages: messages, MaxTokens: maxTokens})
	if err != nil {
		record.SetError(err, time.Since(started))
		return fmt.Errorf("generate change impact analysis: %w", err)
	}
	record.SetResponse(resp, time.Since(started))
	a.runner.RecordUsage(resp.Usage)
	body := strings.TrimSpace(llmloop.StripMarkdownFences(resp.Content()))
	if body == "" {
		return errors.New("generate change impact analysis: LLM returned empty content")
	}
	a.changeAnalysis = body
	return nil
}

func (a *Agent) changeAnalysisInput() (string, string) {
	...
	for _, item := range a.diffs {
		...
		fmt.Fprintf(&files, "- %s\n", path)
		section := fmt.Sprintf("\n### %s\n%s\n", path, item.Diff)
		remaining := maxDiffBytes - diffs.Len()
		if remaining <= 0 {
			break
		}
		if len(section) > remaining {
			diffs.WriteString(strings.ToValidUTF8(section[:remaining], ""))
			diffs.WriteString("\n[diff input truncated]\n")
			break
		}
		diffs.WriteString(section)
	}
	return strings.TrimSpace(files.String()), strings.TrimSpace(diffs.String())
}
