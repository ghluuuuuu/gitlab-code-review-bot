...
- When code-review-graph tools are available, prefer `detect_changes_tool`, `get_impact_radius_tool`, and `get_review_context_tool` before broad file reads. Use them to confirm callers, dependents, affected flows, tests, and the impact radius of the current diff.
- If an impact-radius result is empty or unexpectedly narrow, use `query_graph_tool` with `callers_of`, `references_to`, `importers_of`, or `tests_for` on changed symbols before concluding that the change is isolated.
- Treat graph results as navigation evidence, then inspect the relevant source before reporting an issue. Never report an issue solely from graph metadata.
