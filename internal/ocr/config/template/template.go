...
type Template struct {
	...
	ChangeAnalysisTask        *LlmConversation `json:"CHANGE_ANALYSIS_TASK,omitempty"`
	FindingReconciliationTask *LlmConversation `json:"FINDING_RECONCILIATION_TASK,omitempty"`
...
type templateManifest struct {
	...
	ChangeAnalysisTask        *manifestConversation `json:"CHANGE_ANALYSIS_TASK,omitempty"`
	FindingReconciliationTask *manifestConversation `json:"FINDING_RECONCILIATION_TASK,omitempty"`
...
func LoadDefault() (*Template, error) {
	...
	if tpl.ChangeAnalysisTask, err = resolveOptionalConversation(m.ChangeAnalysisTask, "CHANGE_ANALYSIS_TASK"); err != nil {
		return nil, err
	}
	if tpl.FindingReconciliationTask, err = resolveOptionalConversation(m.FindingReconciliationTask, "FINDING_RECONCILIATION_TASK"); err != nil {
		return nil, err
	}
...
func (t *Template) ApplyLanguage(lang string) {
	...
	if t.ChangeAnalysisTask != nil {
		applyLanguage(t.ChangeAnalysisTask, instruction)
	}
	if t.FindingReconciliationTask != nil {
		applyLanguage(t.FindingReconciliationTask, instruction)
	}
