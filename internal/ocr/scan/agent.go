...
import (
	...
	allowedext "github.com/ghluuuuuu/gitlab-code-review-bot/internal/ocr/config/allowlist"
...
func (a *Agent) dispatchBatch(ctx context.Context, batchIdx int, batch []model.ScanItem) (int64, bool, error) {
	...
	for i := range batch {
		...
		if item, ok := a.resumeItem(fingerprint); ok {
			...
			a.session.RecordReviewItemReused(it.Path, it.Path, it.Path, fingerprint, sourceSessionID, item.Comments)
			continue
		}
