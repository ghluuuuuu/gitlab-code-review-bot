...
const (
	...
	ChangeAnalysisTask        TaskType = "change_analysis_task"
	ReLocationTask            TaskType = "re_location_task"
	ReviewFilterTask          TaskType = "review_filter_task"
	FindingReconciliationTask TaskType = "finding_reconciliation_task"
)
...
type SessionOptions struct {
	// SessionID continues an existing persisted session when non-empty. This is
	// used by incremental MR reviews so the report URL and complete history stay
	...
	// "review") New creates a ManifestBuilder with the operation and the
	// session ID as its run_id. Empty (the scan/legacy default) leaves the
	// builder nil.
	Operation string
}
...
// New creates a new SessionHistory with the given repo directory. When
// opts.SessionID is set, records are appended to that existing report.
