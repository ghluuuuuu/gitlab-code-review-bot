...
// NewCommentCollectorWithCallback creates a collector that invokes onAdd after
// each comment has been committed. The callback runs without the collector lock.
func NewCommentCollectorWithCallback(onAdd func(model.LlmComment)) *CommentCollector {
	return &CommentCollector{onAdd: onAdd}
}
