...
func parseTemplate(name string) (*template.Template, error) {
	funcMap := template.FuncMap{
		...
		"findingStatusLabel": func(status any) string {
			switch FindingStatus(fmt.Sprint(status)) {
			case FindingNew:
				return "新增缺陷"
			case FindingUnfixed:
				return "未修复"
		...
		"findingStatusClass": func(status any) string {
		...
		"timelineDecisionFinding": func(entry session.FindingTimeline, decision session.FindingDecision) string {
			id := decision.CurrentID
			if id == "" {
				id = decision.HistoricalID
			}
			for _, ref := range entry.Current {
				if ref.ID == id {
					return ref.Comment.Content
				}
			}
