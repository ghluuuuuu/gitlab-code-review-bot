...
type ConfigSection = 'storage' | 'gitlab' | 'review' | 'llm' | 'code_graph' | 'auth' | 'server'
...
const activeSection = ref<ConfigSection>('storage')
...
const activeConfig = () => activeSection.value === 'storage'
  ? { database_path: form.database_path, data_dir: form.data_dir }
  : form[activeSection.value]

const save = async () => {
  saving.value = true
  try {
    const response = await getJSON<{ restart_required: boolean }>('/api/v1/admin/config', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ section: activeSection.value, config: activeConfig() }),
    })
    ElMessage.success(response.restart_required ? `${activeSectionLabel.value}配置已保存，重启服务后生效` : `${activeSectionLabel.value}配置已保存`)
  } catch (error) {
...
<template>
  <div class="page-heading">
    <div>
      <h1>系统配置</h1>
      <p>按功能模块管理 config.json。保存时只更新当前标签页，敏感字段留空时保留当前值。</p>
    </div>
    <el-button type="primary" :loading="saving" :disabled="!configQuery.data.value?.can_save" @click="save">
      保存{{ activeSectionLabel }}
    </el-button>
  </div>
  ...
  <el-tabs v-if="initialized" v-model="activeSection" class="config-tabs">
    <el-tab-pane v-for="(label, section) in sectionLabels" :key="section" :label="label" :name="section">
      <div v-if="section === 'storage'" class="config-grid">
      ...
      <div v-else-if="section === 'gitlab'" class="config-grid">
      ...
      <div v-else-if="section === 'review'" class="config-grid">
      ...
      <div v-else-if="section === 'llm'" class="config-grid">
      ...
      <div v-else-if="section === 'code_graph'" class="config-grid">
      ...
      <div v-else-if="section === 'auth'" class="config-grid">
      ...
      <div v-else-if="section === 'server'" class="config-grid">
...
<style scoped>
.page-heading{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px}.page-heading h1{margin:0 0 7px;font-size:25px}.page-heading p{margin:0;color:#9499ad;font-size:13px}.notice{margin-bottom:18px}.config-tabs :deep(.el-tabs__header){margin-bottom:18px}.config-tabs :deep(.el-tabs__item.is-active){color:#5c52ce;font-weight:700}.config-tabs :deep(.el-tabs__active-bar){background:#665bd9}.config-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;align-items:start}.config-grid .el-card{border:1px solid #ebedf4;border-radius:12px}.row{display:grid;grid-template-columns:1fr 1fr;gap:14px}.el-input-number{width:100%}@media(max-width:900px){.page-heading{gap:16px}.config-grid{grid-template-columns:1fr}}
</style>
